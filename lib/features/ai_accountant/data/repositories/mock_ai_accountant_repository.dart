...
class MockAiAccountantRepository implements AiAccountantRepository {
  ...
  Future<AiProposalModel> parseInvoiceImage(
    ...
    await Future.delayed(
        const Duration(milliseconds: 1200)); // Simulate deep OCR analysis

    return AiProposalModel(
        actionType: 'purchase',
        explanation:
            'تم فحص المستند ضوئياً: فاتورة توريد مواد خام مستخرجة آلياً من الصورة المرفوعة.',
  ...
  Future<ProposalExecutionResult> executeProposalDetailed(
    ...
    return const ProposalExecutionResult(
      ...
      data: {
        'syncQueue': {'status': 'mock'},
        'auditLog': {'status': 'mock'},
      },
    );
  }
}
