...
class AppCopy {
  AppCopy._(this._code);

  final String _code;

  static AppCopy of(BuildContext context) =>
      AppCopy._(Localizations.localeOf(context).languageCode);

  bool get isEnglish => _code == 'en';

  String t(String key) => (_map[_code] ?? _map['ar']!)[key] ?? key;

  String requiredField(String field) =>
      isEnglish ? 'Please enter $field' : 'يرجى إدخال $field';
  ...
  String sellProductTitle(String name) =>
      isEnglish ? 'Sell $name' : 'بيع $name';

  String inventoryResultCount(int count) =>
      isEnglish ? 'Results count: $count' : 'عدد النتائج: $count';

  String inventoryDeleteConfirm(String name) => isEnglish
      ? 'Do you want to permanently delete "$name"?'
      : 'هل تريد حذف الصنف "$name" نهائيًا؟';

  String inventoryUnitLine(String unit) =>
      isEnglish ? 'Unit: $unit' : 'الوحدة: $unit';

  String salesHistoryResultCount(int count) =>
      isEnglish ? 'Results count: $count' : 'عدد النتائج: $count';
  ...
  String salesHistoryCurrency(String value) =>
      isEnglish ? 'Currency: $value' : 'العملة: $value';

  String salesHistoryCustomer(String value) =>
      isEnglish ? 'Customer: $value' : 'العميل: $value';

  String salesHistoryNote(String value) =>
      isEnglish ? 'Note: $value' : 'ملاحظة: $value';
  ...
  String productDetailsQuantityLine(int qty) =>
      isEnglish ? 'Quantity: $qty' : 'الكمية: $qty';

  String productDetailsCustomerLine(String name) =>
      isEnglish ? 'Customer: $name' : 'العميل: $name';

  String productDetailsMovementBalanceLine(int balance) => isEnglish
      ? 'Balance after movement: $balance'
      : 'الرصيد بعد الحركة: $balance';
  ...
  String dashboardLastRestore(String value) =>
      isEnglish ? 'Last restore: $value' : 'آخر استعادة: $value';

  String dashboardStockLine(int stockQty, String unit, int threshold) =>
      isEnglish
          ? 'Stock $stockQty $unit - threshold $threshold'
          : 'الرصيد $stockQty من $unit - الحد $threshold';

  String dashboardRecentSaleSubtitle({
  ...
  String documentsDeleteQuotationSuccess() =>
      isEnglish ? 'Quotation deleted successfully.' : 'تم حذف عرض السعر بنجاح.';
  ...
  static const Map<String, Map<String, String>> _map = {
    'ar': {
      ...
      'logout': 'تسجيل الخروج',
      'overview': 'نظرة عامة',
      'overviewSubtitle': 'ملخص سريع للحالة التشغيلية الحالية.',
      'totalProducts': 'إجمالي الأصناف',
      'totalSales': 'إجمالي المبيعات',
      'estimatedProfit': 'الربح المتوقع',
      'lowStockCount': 'منخفضة المخزون',
      'noAlertsNow': 'لا توجد تنبيهات حاليًا',
      'needsAttentionSoon': 'تحتاج متابعة قريبة',
      'quickActions': 'إجراءات سريعة',
      'quickActionsSubtitle': 'أهم العمليات اليومية',
      ...
      'documentsErrorInvoiceNotFound': 'تعذر العثور على الفاتورة.',
      'documentsErrorQuotationNotFound': 'تعذر العثور على عرض السعر.',
      'documentsTabInvoices': 'الفواتير',
      'documentsTabQuotations': 'عروض الأسعار',
      'inventoryTitle': 'المخزون',
      'searchInventoryHint': 'ابحث باسم الصنف أو الوحدة أو الباركود',
      'searchInventoryHelp': 'يمكن لصق الباركود للوصول إلى الصنف مباشرة.',
      'productFilter': 'فلترة الأصناف',
      'sortBy': 'الترتيب حسب',
      'ascendingSort': 'ترتيب تصاعدي',
      'noMatchingProducts': 'لا توجد أصناف مطابقة للبحث الحالي.',
      'stock': 'المخزون',
      ...
      'cannotDeleteProduct': 'لا يمكن حذف الصنف.',
      'productDeleted': 'تم حذف الصنف بنجاح.',
      ...
      'realizedProfit': 'إجمالي الربح المحقق',
      'stockMovements': 'آخر حركات المخزون',
      'stockMovementsSubtitle': 'أحدث التغيرات على رصيد الصنف.',
      'noStockMovements': 'لا توجد حركات مخزون مسجلة لهذا الصنف.',
      'latestSales': 'آخر عمليات البيع',
      'latestSalesSubtitle': 'أحدث السجلات البيعية الخاصة بهذا الصنف.',
      'noProductSales': 'لا توجد عمليات بيع مسجلة لهذا الصنف.',
      'adjustStockOrCost': 'تسوية مخزون أو تكلفة',
      'loadProductDetailsError': 'تعذر تحميل تفاصيل الصنف.',
      'openingBalance': 'رصيد افتتاحي',
      'sale': 'بيع',
      'invoice': 'فاتورة',
      'costAdjustment': 'تسوية تكلفة',
      'restore': 'استعادة',
      'movement': 'حركة',
      'debit': 'مدين',
      'credit': 'دائن',
      'salesHistoryTitle': 'سجل المبيعات',
      'loadSalesHistoryError': 'تعذر تحميل سجل المبيعات.',
      'searchSalesHint': 'ابحث باسم الصنف أو العميل',
      'recordPeriod': 'فترة السجل',
      'period': 'الفترة',
      'noMatchingSales': 'لا توجد عمليات بيع مطابقة للفلاتر الحالية.',
      'allPeriods': 'كل الفترات',
      'today': 'اليوم',
      'last7Days': 'آخر 7 أيام',
      'last30Days': 'آخر 30 يومًا',
      'sellSuccess': 'تم تسجيل عملية البيع بنجاح.',
      'sellError': 'تعذر إتمام عملية البيع.',
      'sellInvalidQty': 'يرجى إدخال كمية صحيحة أكبر من صفر.',
      'sellQtyTooHigh': 'الكمية المطلوبة أكبر من المخزون المتاح.',
      'sellNegativePrice': 'سعر البيع يجب ألا يكون قيمة سالبة.',
      'currencyOptional': 'العملة (اختياري)',
      'currencyExample': 'مثال: USD أو EUR أو TRY',
      'noCurrencySpecified': 'بدون عملة محددة',
      'customerNameOptional': 'اسم العميل (اختياري)',
      'noteOptional': 'ملاحظة (اختياري)',
      'saleTotal': 'إجمالي البيع',
      'expectedProfit': 'الربح المتوقع',
      'confirmSale': 'تأكيد البيع',
      'submittingSale': 'جارٍ تسجيل البيع...',
      'reportsTitle': 'التقارير والتحليلات',
      'loadReportsError': 'تعذر تحميل التقارير.',
      ...
      'bestSellingProducts': 'أفضل الأصناف مبيعًا',
      'noBestSellingData': 'لا توجد بيانات مبيعات كافية بعد.',
      'lowStockProducts': 'الأصناف منخفضة المخزون',
      'accountingSection': 'قسم المحاسبة',
      'accountingBalanced': 'الميزان متوازن حاليًا.',
      'accountingNeedsReview': 'الميزان يحتاج مراجعة.',
      'filterByProduct': 'تصفية حسب الصنف',
      'allProducts': 'كل الأصناف',
      'export': 'التصدير',
      'inventoryPdf': 'PDF المخزون',
      'salesPdf': 'PDF المبيعات',
      'inventoryCsv': 'CSV المخزون',
      'salesCsv': 'CSV المبيعات',
      'exportPdfDone': 'تم تصدير ملف PDF',
      'previewNowOrShare': 'هل تريد معاينة الملف أو مشاركته الآن؟',
      'later': 'لاحقًا',
      'share': 'مشاركة',
      'preview': 'معاينة',
      ...
      'businessProfileTitle': 'ملف الشركة',
      'addProductTitle': 'إضافة صنف',
      'productName': 'اسم الصنف',
      'unit': 'الوحدة',
      'saveSuccessProduct': 'تم حفظ الصنف بنجاح.',
      'saveErrorProduct': 'تعذر حفظ الصنف.',
      'invalidValue': 'قيمة غير صحيحة',
      'customersTitle': 'العملاء',
      'addCustomerTitle': 'إضافة عميل',
      'editCustomerTitle': 'تعديل عميل',
      'phone': 'الهاتف',
      'addressOrNotes': 'العنوان أو الملاحظات',
      'customerAdded': 'تمت إضافة العميل بنجاح.',
      'customerUpdated': 'تم تحديث بيانات العميل.',
      'loadCustomersError': 'تعذر تحميل قائمة العملاء.',
      'noCustomersYet': 'لا يوجد عملاء بعد.',
      'notes': 'ملاحظات',
      'customerInvoices': 'الفواتير',
      'customerPayments': 'الدفعات',
      'noCustomerInvoices': 'لا توجد فواتير لهذا العميل بعد.',
      'noCustomerPayments': 'لا توجد دفعات مسجلة لهذا العميل بعد.',
      'status': 'الحالة',
      ...
      'logoHelp': 'يفضل صورة مربعة أو بخلفية شفافة. سيتم عرضها كاملة بدون قص.',
      'basicInfo': 'البيانات الأساسية',
      'contactInfo': 'بيانات التواصل',
      'officialInfo': 'البيانات الرسمية',
      'documentDefaults': 'الإعدادات الافتراضية للمستندات',
      ...
      'registrationNumber': 'السجل',
      'quotationNotes': 'ملاحظات عرض السعر',
      'invoiceNotes': 'ملاحظات الفاتورة',
      'paymentTerms': 'شروط الدفع',
      'invoiceDetailsTitle': 'تفاصيل الفاتورة',
      'invoiceNotFound': 'تعذر العثور على الفاتورة.',
      'customer': 'العميل',
      'invoiceDate': 'تاريخ الفاتورة',
      'dueDate': 'تاريخ الاستحقاق',
      'currency': 'العملة',
      'items': 'البنود',
      'noItemsAvailable': 'لا توجد بنود متاحة.',
      'qtyPriceLine': 'الكمية: {qty} × {price}',
      ...
      'quotationItemsConverted': 'بنود عرض السعر المحوّلة',
      'product': 'الصنف',
      'unitPrice': 'سعر الوحدة',
      'addItem': 'إضافة بند',
      'noItemsYet': 'لا توجد بنود مضافة بعد.',
      'paymentAndNotes': 'الدفع والملاحظات',
      'invoiceCreated': 'تم إنشاء الفاتورة',
      'previewOrShareNow': 'هل تريد معاينتها أو مشاركتها الآن؟',
      'customerRequired': 'يرجى اختيار العميل.',
      'atLeastOneItem': 'يجب إضافة بند واحد على الأقل.',
      'partialPaymentValid': 'أدخل قيمة دفعة جزئية صحيحة أو غيّر نوع الدفع.',
      'invoiceDraftSaved': 'تم حفظ الفاتورة كمسودة.',
      'invoiceSavedPdf': 'تم إصدار الفاتورة وحفظ ملف PDF.',
      'invoiceSavedNotFound': 'تعذر العثور على الفاتورة بعد حفظها.',
      'pdfMissing': 'تعذر العثور على ملف PDF.',
      ...
      'previewOrShareQuotationNow': 'هل تريد معاينته أو مشاركته الآن؟',
      'createQuotationTitle': 'إنشاء عرض سعر',
      'quotationInfo': 'بيانات العرض',
      'issueDate': 'تاريخ الإصدار',
      'expiryDate': 'تاريخ الانتهاء',
      'saveQuotationPdf': 'حفظ العرض وإنشاء PDF',
      'quotationItemsTitle': 'بنود العرض',
      'notesAndSummary': 'ملاحظات وملخص',
      'quotationSavedPdf': 'تم حفظ عرض السعر وإنشاء ملف PDF.',
      'quotationDetailsTitle': 'تفاصيل عرض السعر',
      'quotationNotFound': 'تعذر العثور على عرض السعر.',
      'quotationCannotBeConverted': 'لا يمكن تحويل عرض السعر هذا إلى فاتورة.',
      'statusDraft': 'مسودة',
      'statusSent': 'مرسل',
      ...
      'adjustSingleMode': 'أجرِ تسوية واحدة في كل مرة: كمية أو تكلفة.',
      'adjustStockQty': 'تسوية كمية',
      'adjustCost': 'تسوية تكلفة',
      'newBalance': 'الرصيد الجديد',
      'currentBalance': 'الرصيد الحالي: {value}',
      'newPurchasePrice': 'سعر الشراء الجديد',
      'currentValue': 'الحالي: {value}',
      'newExtraCosts': 'المصاريف الإضافية الجديدة',
      'adjustReason': 'سبب التسوية',
      'adjustReasonHelp': 'مطلوب لتوثيق الحركة والقيود المحاسبية.',
      ...
      'syncCenterSubtitle': 'مراقبة حالة البيانات والربط السحابي.',
      'syncStatus': 'حالة المزامنة',
      'syncStatsPending': 'قيد الانتظار',
      'syncStatsFailed': 'فشلت',
      'syncStatsSyncedToday': 'تمت اليوم',
      'syncStatsConflicts': 'تعارضات',
      'syncLastSync': 'آخر مزامنة: {time}',
      'syncNeverSynced': 'لم تتم المزامنة بعد',
      'syncQueueTitle': 'طابور العمليات',
      'syncTimelineTitle': 'سجل النشاط',
      'syncRetryAll': 'إعادة محاولة الكل',
      'syncClearSynced': 'مسح المكتمل',
      'syncOffline': 'غير متصل',
      'syncOnline': 'متصل',
      'syncHealthGood': 'المزامنة مستقرة',
      'syncHealthWarning': 'توجد أخطاء تحتاج مراجعة',
      'syncHealthPending': 'توجد عمليات معلقة',
      'syncOperationRetry': 'إعادة المحاولة',
      'syncOperationRemove': 'إزالة',
      'syncOperationDetails': 'تفاصيل العملية',
      'syncEntityType': 'نوع الكيان',
      'syncOpType': 'نوع العملية',
      'syncCreated': 'تم الإنشاء',
      'syncLastAttempt': 'آخر محاولة',
      'syncRetryCount': 'محاولات: {count}',
      'syncBranch': 'الفرع: {name}',
      'sku': 'رمز الصنف (SKU)',
      'category': 'التصنيف',
      'brand': 'العلامة التجارية',
      'supplier': 'المورد',
      'description': 'الوصف',
      'pricing': 'التسعير والضرائب',
      'inventory': 'المخزون والمستودع',
      'attributes': 'المواصفات والسمات',
      'productSettings': 'خيارات البيع والظهور',
      'advanced': 'خيارات متقدمة',
      'pricingSubtitle': 'ضبط أسعار البيع والجملة والخصومات والضرائب.',
      'inventorySubtitle': 'إدارة مستويات المخزون ومواقع التخزين والتنبيهات.',
      'attributesSubtitle': 'إضافة تفاصيل تقنية مثل اللون والمقاس والوزن.',
      'settingsSubtitle': 'التحكم في حالة الصنف وطريقة ظهوره في النظام.',
      'image': 'صورة الصنف',
      'gallery': 'معرض الصور',
      ...
      'internalNotes': 'ملاحظات داخلية (غير معلنة)',
      'isSellable': 'متاح للبيع',
      'isDiscountAllowed': 'يسمح بالخصم عليه',
      'isTrackingEnabled': 'تفعيل تتبع المخزون',
      'showInReports': 'يظهر في التقارير',
      'requiresSerial': 'يتطلب رقم تسلسلي عند البيع',
      'requiresExpiry': 'يتطلب تاريخ انتهاء',
      'isFeatured': 'صنف مميز (Featured)',
      'isHidden': 'إخفاء من قائمة البيع',
      'profitCalculation': 'حساب الأرباح',
      'netProfit': 'صافي الربح',
      'margin': 'الهامش',
      'stockHealth': 'سلامة المخزون',
      'productPreview': 'معاينة الصنف',
    ...
    'en': {
      ...
      'logout': 'Log Out',
      'overview': 'Overview',
      'overviewSubtitle': 'A quick summary of the current operational status.',
      'totalProducts': 'Total Products',
      'totalSales': 'Total Sales',
      'estimatedProfit': 'Estimated Profit',
      'lowStockCount': 'Low Stock Items',
      'noAlertsNow': 'No alerts right now',
      'needsAttentionSoon': 'Needs attention soon',
      'quickActions': 'Quick Actions',
      'quickActionsSubtitle': 'Most common daily actions',
      ...
      'documentsErrorInvoiceNotFound': 'Failed to find the invoice.',
      'documentsErrorQuotationNotFound': 'Failed to find the quotation.',
      'documentsTabInvoices': 'Invoices',
      ...
      'noMatchingProducts': 'No products match the current search.',
      'stock': 'Stock',
      ...
      'cannotDeleteProduct': 'This product cannot be deleted.',
      ...
      'adjustStockOrCost': 'Adjust Stock or Cost',
      'loadProductDetailsError': 'Failed to load product details.',
      ...
      'loadSalesHistoryError': 'Failed to load sales history.',
      'searchSalesHint': 'Search by product name or customer',
      'recordPeriod': 'Record Period',
      'period': 'Period',
      'noMatchingSales': 'No sales match the current filters.',
      'allPeriods': 'All Periods',
      'today': 'Today',
      'last7Days': 'Last 7 Days',
      'last30Days': 'Last 30 Days',
      'sellSuccess': 'Sale recorded successfully.',
      'sellError': 'Failed to complete the sale.',
      'sellInvalidQty': 'Please enter a valid quantity greater than zero.',
      'sellQtyTooHigh': 'Requested quantity is greater than available stock.',
      'sellNegativePrice': 'Selling price cannot be negative.',
      'currencyOptional': 'Currency (optional)',
      'currencyExample': 'Example: USD, EUR, or TRY',
      'noCurrencySpecified': 'No specific currency',
      'customerNameOptional': 'Customer name (optional)',
      'noteOptional': 'Note (optional)',
      'saleTotal': 'Sale Total',
      'expectedProfit': 'Expected Profit',
      'confirmSale': 'Confirm Sale',
      'submittingSale': 'Recording sale...',
      ...
      'noBestSellingData': 'There is not enough sales data yet.',
      'lowStockProducts': 'Low-Stock Products',
      'accountingSection': 'Accounting',
      'accountingBalanced': 'The balance is currently balanced.',
      'accountingNeedsReview': 'The balance needs review.',
      ...
      'previewNowOrShare': 'Would you like to preview or share the file now?',
      'later': 'Later',
      'share': 'Share',
      'preview': 'Preview',
      ...
      'unit': 'Unit',
      'saveSuccessProduct': 'Product saved successfully.',
      'saveErrorProduct': 'Failed to save product.',
      'invalidValue': 'Invalid value',
      'customersTitle': 'Customers',
      'addCustomerTitle': 'Add Customer',
      'editCustomerTitle': 'Edit Customer',
      'phone': 'Phone',
      'addressOrNotes': 'Address or notes',
      ...
      'loadCustomersError': 'Failed to load customers list.',
      'noCustomersYet': 'There are no customers yet.',
      'notes': 'Notes',
      ...
      'documentDefaults': 'Default Document Settings',
      ...
      'qtyPriceLine': 'Quantity: {qty} × {price}',
      ...
      'quotationItemsConverted': 'Converted Quotation Items',
      'product': 'Product',
      'unitPrice': 'Unit Price',
      'addItem': 'Add Item',
      'noItemsYet': 'No items added yet.',
      'paymentAndNotes': 'Payment and Notes',
      ...
      'pdfMissing': 'PDF file was not found.',
      'customerNameRequired': 'Customer name is required.',
      'customerCreated': 'Customer added successfully.',
      'pickDate': 'Choose date',
      'optional': 'Optional',
      'quotationCreated': 'Quotation Created',
      'previewOrShareQuotationNow': 'Do you want to preview or share it now?',
      'createQuotationTitle': 'Create Quotation',
      'quotationInfo': 'Quotation Information',
      'issueDate': 'Issue Date',
      'expiryDate': 'Expiry Date',
      'saveQuotationPdf': 'Save Quotation and Create PDF',
      'quotationItemsTitle': 'Quotation Items',
      'notesAndSummary': 'Notes and Summary',
      ...
      'adjustSingleMode': 'Apply one adjustment at a time: quantity or cost.',
      'adjustStockQty': 'Adjust Quantity',
      ...
      'syncCenterSubtitle': 'Monitor data health and cloud connectivity.',
      'syncStatus': 'Sync Status',
      'syncStatsPending': 'Pending',
      'syncStatsFailed': 'Failed',
      'syncStatsSyncedToday': 'Synced Today',
      'syncStatsConflicts': 'Conflicts',
      'syncLastSync': 'Last sync: {time}',
      'syncNeverSynced': 'Never synced',
      'syncQueueTitle': 'Operations Queue',
      'syncTimelineTitle': 'Activity Log',
      'syncRetryAll': 'Retry All',
      'syncClearSynced': 'Clear Synced',
      'syncOffline': 'Offline',
      'syncOnline': 'Online',
      'syncHealthGood': 'Sync is stable',
      'syncHealthWarning': 'Issues need review',
      'syncHealthPending': 'Operations pending',
      'syncOperationRetry': 'Retry',
      'syncOperationRemove': 'Remove',
      'syncOperationDetails': 'Operation Details',
      'syncEntityType': 'Entity Type',
      'syncOpType': 'Op Type',
      'syncCreated': 'Created',
      'syncLastAttempt': 'Last Attempt',
      ...
      'isSellable': 'Available for Sale',
      'isDiscountAllowed': 'Allow Discounts',
      'isTrackingEnabled': 'Enable Stock Tracking',
      'showInReports': 'Show in Reports',
      'requiresSerial': 'Requires Serial on Sale',
      'requiresExpiry': 'Requires Expiry Date',
      'isFeatured': 'Featured Product',
      'isHidden': 'Hide from Sales List',
