...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  Future<void> _refresh() async {
    await Future.wait([_repo.getInvoices(), _repo.getQuotations()]);
  }
  ...
  Future<String> _invoicePdf(Map<String, dynamic> invoice) async {
    ...
    if (fresh == null) throw Exception(copy.t('documentsErrorInvoiceNotFound'));
  ...
  Future<String> _quotationPdf(Map<String, dynamic> quotation) async {
    ...
    if (fresh == null) throw Exception(copy.t('documentsErrorQuotationNotFound'));
  ...
  Future<void> _preview(String path) async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => _PdfPreviewScreen(title: p.basename(path), bytes: bytes),
      ),
    );
  }
  ...
  Future<void> _deleteInvoice(String id) async {
    ...
    if (!mounted) return;
    AppMessages.success(context, AppCopy.of(context).documentsDeleteInvoiceSuccess());
  }

  Future<void> _deleteQuotation(String id) async {
    await _repo.deleteQuotation(id);
    if (!mounted) return;
    AppMessages.success(context, AppCopy.of(context).documentsDeleteQuotationSuccess());
  }
              ...
              content: Column(
                ...
                children: [
                  TextField(
                    ...
                    decoration: InputDecoration(labelText: copy.t('amount')),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    value: paymentMethod,
                    decoration: InputDecoration(labelText: copy.t('paymentMethod')),
                    items: [
                      DropdownMenuItem(value: 'cash', child: Text(copy.t('cash'))),
                      DropdownMenuItem(
                        value: 'bank_transfer',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(_showInvoices ? copy.t('newInvoice') : copy.t('quotations')),
      ),
                ...
                Wrap(
                  ...
                  children: [
                    ChoiceChip(
                      ...
                      onSelected: (_) => setState(() => _showInvoices = true),
                    ),
                    ChoiceChip(
                      label: Text(copy.t('documentsTabQuotations')),
                      selected: !_showInvoices,
                      onSelected: (_) => setState(() => _showInvoices = false),
                    ),
                  ],
                ),
                ...
                if (rows.isEmpty)
                  Card(
                    child: Padding(
                      ...
                      child: Center(child: Text(copy.t('noDocuments'))),
                    ),
                  )
                else
                  ...rows.map(
                    (row) => Padding(
                      ...
                      child: _showInvoices ? _invoiceCard(row, copy) : _quotationCard(row, copy),
                    ),
                  ),
              ],
            );
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _chip(copy.t('total'), AppFormatters.currency(total, currencyLabel: currencyCode)),
                _chip(copy.t('paid'), AppFormatters.currency(paid, currencyLabel: currencyCode)),
                _chip(
                  copy.t('remaining'),
  ...
  Widget _chip(String label, String value) {
    return Container(
      ...
      child: Text(AppCopy.of(context).documentsChipLine(label, value)),
    );
  }
